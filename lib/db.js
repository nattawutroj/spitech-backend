...
function getStaff(callback) {
    client.query(`SELECT id_staff, initials, id_name_title, first_name_th, last_name_th, first_name_en, last_name_en, phone, email, id_role
	FROM public."Staff";`, (err, result) => {
        console.log(err)
        if (typeof result == 'undefined') {
            callback(422)
        } else {
...
function getprojectstaff(build, callback) {
            ...
            /*  rows: [
    {
      ...
      id_name_title: 1,
      first_name_th: '(Su) ณัฐวุฒิ',
      first_name_en: '(SU) NATTAWUT',
      name_title_th: 'นาย',
      name_title_en: 'Mr.',
