...
JOIN public."Project" pj ON rp.id_project = pj.id_project
...
function getreqproject(build, callback) {
    console.log(build)
    var command = `SELECT DISTINCT ON (rp.id_project)
    rp.*,
    rst.*,
    pj.*,
    pm.*,
	ss.*
FROM public."Project_Status" rp
LEFT JOIN public."Project_Status_Title" rst ON rp.id_project_status_title = rst.id_project_status_title
LEFT JOIN public."Project" pj ON rp.id_project = pj.id_project
LEFT JOIN public."Project_Member" pm ON rp.id_project = pm.id_project
JOIN public."Student" ss ON pm.id_student =  ss.id_student
ORDER BY rp.id_project`
    console.log(command)
    client.query(command, (err, result) => {
        console.log(err)
        if (typeof result == 'undefined') {
...
function getteacherreqproject(build, callback) {
    console.log(build)
    var command = `SELECT DISTINCT ON (stm.id_project_staff)
	stm.*,
	ps.*,
	st.*,
    rp.*,
    ...
    pm.*,
	ss.*
FROM public."Project_Staff" stm
JOIN public."Project_Staff_Position" ps ON stm.id_project_staff_position = ps.id_project_staff_position
JOIN public."Staff" st ON stm.id_staff = st.id_staff
LEFT JOIN public."Project_Status" rp ON stm.id_project = rp.id_project
LEFT JOIN public."Project_Status_Title" rst ON rp.id_project_status_title = rst.id_project_status_title
LEFT JOIN public."Project" pj ON rp.id_project = pj.id_project
LEFT JOIN public."Project_Member" pm ON rp.id_project = pm.id_project
JOIN public."Student" ss ON pm.id_student =  ss.id_student
WHERE stm.id_staff = ${build.id_staff}`
    console.log(command)
    client.query(command, (err, result) => {
        console.log(err)
        if (typeof result == 'undefined') {
...
function aagetsemeter(callback) {
    command = `SELECT * FROM public."Semester"
...
function getfilehistory(build, callback) {
    var command = `SELECT * FROM public."Project_File_Paths" WHERE id_project = '${build.id_project}'`
    client.query(command, (err, result) => {
        if (typeof result == 'undefined') {
            callback(err)
        } else {
...
function getschedule(callback) {
	...
	pj.*
FROM public."Schedule" sc
JOIN public."Test_Category" tc ON sc.id_test_category = tc.id_test_category
JOIN public."Room" rm ON sc.id_room = rm.id_room
JOIN public."Project" pj ON sc.id_project = pj.id_project`
    client.query(command, (err, result) => {
        if (typeof result == 'undefined') {
            callback(err)
        } else {
...
function getfefile(build, callback) {
    var command = `SELECT *
    FROM public."Project_File_Paths"
    WHERE id_project = '${build.id_project}'
...
module.exports = {
    ...
    getteacherreqproject:getteacherreqproject,
    aagetsemeter: aagetsemeter,
    getfilehistory: getfilehistory,
    delroom: delroom,
    getreqproject: getreqproject,
